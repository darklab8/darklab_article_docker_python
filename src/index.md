...
Для сборки докер контейнера на нужно учесть следующие моменты
...
Почти не работал с Apache... однако если загуглить это понятие, то получается это аналог nginx server синтактсису?

И тогда всплывает вопрос 
...
В чем смысл тогда этого вопроса, если данная директива используется Nginx/Apache чтобы слушать разные цели ip:port и перенаправлять на разные static assets ресурсы, на разные прокси пассы в режиме работы реверс прокси, ну или служа веб сервером обслуживать запросы и выполнением php/python/whatever напрямую?

Этот вопрос не относится к деталям контейнеризации проекта на любом языке программирования
если правильно загуглил ответ.

Однако частым решением деплоя питоновский приложений является запуск отдельным процессом питон вебсервера

через Gunicorn (для sync/wsgi фреймворков), через Uvicorn or Gunicorn+Uvicorn (для async/asgi фреймворкв)

Дополнительно используется Nginx для serving static assets (html/css/js), а так же как реверс прокси в питоновскому веб серверу (Gunicorn к примеру) и дополнительной модификации headers ради client side caching, или server side caching и тд

Аналогично Apache может служить для тех же самых целей. (при большой фантазии Apache с модулем на питон может служить даже веб сервером питоновских файлов, но в реальности это мало популярное решение) (модуль `mod_wsgi`)
